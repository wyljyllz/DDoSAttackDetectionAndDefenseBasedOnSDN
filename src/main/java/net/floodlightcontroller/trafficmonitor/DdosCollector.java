...
public class DdosCollector {
	protected static final Logger logger = LoggerFactory.getLogger(DdosCollector.class);
	
	private  IOFSwitchService 				switchService;
	private  IThreadPoolService 			threadPoolService;	// 线程池
	private  ILinkDiscoveryService			linkDiscoveryService;
	public static long portStatsInterval = 5;					// 收集交换机端口统计量的周期,单位为秒
			...
			if(!replies.isEmpty()){
				...
				if(isFirstTime2CollectSwitchStatistics){
					...
					*/
					...
					DdosAnalyzer.Analysis(prePortStatsBuffer, abnormalTraffic, policy);
					if(!abnormalTraffic.isEmpty()){
						DdosController.executePolicy(switchService, linkDiscoveryService, abnormalTraffic, addFlowEntriesHistory, policy, events);
					}
